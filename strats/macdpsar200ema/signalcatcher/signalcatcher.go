...
func CanEnterLong(model *model.DataModel, broker *api.AlpacaBroker, isCrypto bool) bool {
	if !isCrypto {
		...
		if !model.Position.HasLongPosition && !model.Position.HasShortPosition &&
			model.Signal.CurrentCryptoBar.Close > model.Signal.CurrentEMA200Period && // the current price is above the 200 period EMA value
			model.Signal.CurrentParabolicSar < model.Signal.CurrentCryptoBar.Low &&
			model.Signal.PreviousParabolicSar > model.Signal.PreviousCryptoBar.High &&
			model.Signal.CurrentMacd > model.Signal.CurrentMacdSignal {
			return true
		}
	}
...
func CanEnterShort(model *model.DataModel, broker *api.AlpacaBroker, isCrypto bool) bool {
	if !isCrypto {
		if !model.Position.HasLongPosition && !model.Position.HasShortPosition &&
